...
public class RoundedImageViewTemp extends ImageView {
    ...
    public RoundedImageViewTemp(Context context) {
        super(context);
        init();
    }

    public RoundedImageViewTemp(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
        extractAttributes(attrs);
    }

    public RoundedImageViewTemp(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initClipPath() {
        ...
        if (mIsCircle) {
            ...
            /*Bitmap shaderBmp = Bitmap.createBitmap((int) radius, (int)radius, Bitmap.Config.RGB_565);
            Canvas canvas = new Canvas(shaderBmp);
            Paint p = new Paint(Paint.ANTI_ALIAS_FLAG);
            p.setColor(Color.BLUE);
            p.setStyle(Paint.Style.FILL);
            canvas.drawCircle(leftPadding + realCenterX, topPadding + realCenterY, radius, p);
            mTempPaint.setShader(new BitmapShader(shaderBmp, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP));*/

            mClipPath.addCircle(leftPadding + realCenterX, topPadding + realCenterY, radius, Path.Direction.CCW);

            Bitmap bmp = this.bmp.copy(this.bmp.getConfig(), true);
            this.bmp = bmp;
...
//        canvas.drawPath(mClipPath, mPaint);
        if(bmp != null)
            canvas.drawBitmap(bmp, getImageMatrix(), null);
    }
}